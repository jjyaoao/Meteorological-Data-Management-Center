...
void _tcpgetfiles(){
  ...
  while(true){
    ...
    PActive.UptATime();
    // 接受服务端的报文,MD这个地方没有timetvl了，真tm累啊改错
...
bool  RecvFile(const int sockfd, const char *filename, const char *mtime, int filesize){
  ...
  // 重置文件的时间, 文件原始的时间更有意义，因此要重置，不重置就是传输之后的时间
