/*
 * 程序名：dminingmysql.cpp，此程序是数据中心的公共功能模块，用于从mysql数据库源表抽取数据，生成xml文件
 * 作者：jjyaoao
*/
#include "_public.h"
#include "_mysql.h"
...
bool _dminingmysql(){
	...
	while (true){
		...
		if ((starg.maxcount>0) && (stmt.m_cda.rpc%starg.maxcount == 0) ){
			...
			// 关闭成功，写日志
			logfile.Write("生成文件%s(starg.maxcount). \n", strxmlfilename);
...
bool _xmltoarg(char *strxmlbuffer){
	...
	GetXMLBuffer(strxmlbuffer,"selectsql",starg.selectsql,starg.maxcount);
