...
struct st_arg{		//注意都多了1，因为char数组最后要留给\0一个位置
  ...
  int  ptype;		   // 下载后服务器文件的处理方式：1-什么都不做；2-删除；3-备份
...
void _help()
{
  ...
  printf("<matchname>*.txt</matchname> 待下载文件匹配的规则。"\
...
bool _xmltoarg(char *strxmlbuffer){
	...
	memset(&starg, 0, sizeof(struct st_arg));
		...
		{ logfile.Write("localpath is null.\n");  return false; }
		 ...
		 // 下载后服务器文件的处理方式：1-什么都不做；2-删除；3-备份
		...
		if((starg.ptype != 1) && (starg.ptype != 2) && (starg.ptype != 3)){
		 ...
		 GetXMLBuffer(strxmlbuffer,"remotepathbak",starg.remotepathbak, 300);   // 下载前列出服务器文件名的文件
                 if ((starg.ptype == 3) && strlen(starg.remotepathbak) == 0) {//多加了一个判断，只有ptype为3，才判断是否为空
...
bool _ftpgetfiles(){
	...
	while(true){
		...
		if(File.Fgets(stfileinfo.filename, 300, true) == false) break;	// 读取一个长度为300的char形数组filename
  ...
  bool LoadOKFile(){
	vlistfile1.clear();
	CFile File;
	// 注意：如果程序第一次下载，okfilename是不存在的，并不是错误，所以也要返回true
